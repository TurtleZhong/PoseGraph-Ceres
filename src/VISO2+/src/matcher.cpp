...
pushBack (uint8_t *I1,uint8_t* I2,int32_t* dims,const bool replace) {
    ...
    dims_c[0] = width;
    dims_c[1] = height;
    dims_c[2] = width + 15-(width-1)%16; //1248 for KITTI dataset
...
//        cv::Mat test(dims_c[1],dims_c[2],CV_8UC1,I1c);
//        cv::imshow("test",test);
    /*test suscessfully*/
...
inline void Matcher::computeDescriptor (const uint8_t* I_du,const uint8_t* I_dv,const int32_t &bpl,const int32_t &u,const int32_t &v,uint8_t *desc_addr) {
    ...
    // compute descriptor
    uint32_t k = 0;
    desc_addr[k++] = I_du[addr_m1-3]; //cout << endl << "I_du[addr_m1-3] = " << (int)I_du[addr_m1-3] << endl;
    desc_addr[k++] = I_dv[addr_m1-3]; //cout << endl << "I_dv[addr_m1-3] = " << (int)I_dv[addr_m1-3] << endl;
    desc_addr[k++] = I_du[addr_p1-3]; //cout << endl << "I_du[addr_p1-3] = " << (int)I_du[addr_p1-3] << endl;
    desc_addr[k++] = I_dv[addr_p1-3]; //cout << endl << "I_dv[addr_p1-3] = " << (int)I_dv[addr_p1-3] << endl;
    desc_addr[k++] = I_du[addr_m1-1]; //cout << endl << "I_du[addr_m1-1] = " << (int)I_du[addr_m1-1] << endl;
    desc_addr[k++] = I_dv[addr_m1-1]; //cout << endl << "I_dv[addr_m1-1] = " << (int)I_dv[addr_m1-1] << endl;
    desc_addr[k++] = I_du[addr_p1-1]; //cout << endl << "I_du[addr_p1-1] = " << (int)I_du[addr_p1-1] << endl;
    desc_addr[k++] = I_dv[addr_p1-1]; //cout << endl << "I_dv[addr_p1-1] = " << (int)I_dv[addr_p1-1] << endl;
    desc_addr[k++] = I_du[addr_m1+3]; //cout << endl << "I_du[addr_m1+3] = " << (int)I_du[addr_m1+3] << endl;
    desc_addr[k++] = I_dv[addr_m1+3]; //cout << endl << "I_dv[addr_m1+3] = " << (int)I_dv[addr_m1+3] << endl;
    desc_addr[k++] = I_du[addr_p1+3]; //cout << endl << "I_du[addr_p1+3] = " << (int)I_du[addr_p1+3] << endl;
    desc_addr[k++] = I_dv[addr_p1+3]; //cout << endl << "I_dv[addr_p1+3] = " << (int)I_dv[addr_p1+3] << endl;
    ...
    desc_addr[k++] = I_du[addr_p1+1]; //cout << endl << "I_du[addr_p1+1] = " << (int)I_du[addr_p1+1] << endl;
    desc_addr[k++] = I_dv[addr_p1+1]; //cout << endl << "I_dv[addr_p1+1] = " << (int)I_dv[addr_p1+1] << endl;
    desc_addr[k++] = I_du[addr_m5-1]; //cout << endl << "I_du[addr_m5-1] = " << (int)I_du[addr_m5-1] << endl;
    desc_addr[k++] = I_dv[addr_m5-1]; //cout << endl << "I_dv[addr_m5-1] = " << (int)I_dv[addr_m5-1] << endl;
    desc_addr[k++] = I_du[addr_p5-1]; //cout << endl << "I_du[addr_p5-1] = " << (int)I_du[addr_p5-1] << endl;
    desc_addr[k++] = I_dv[addr_p5-1]; //cout << endl << "I_dv[addr_p5-1] = " << (int)I_dv[addr_p5-1] << endl;
    desc_addr[k++] = I_du[addr_m5+1]; //cout << endl << "I_du[addr_m5+1] = " << (int)I_du[addr_m5+1] << endl;
    desc_addr[k++] = I_dv[addr_m5+1]; //cout << endl << "I_dv[addr_m5+1] = " << (int)I_dv[addr_m5+1] << endl;
    desc_addr[k++] = I_du[addr_p5+1]; //cout << endl << "I_du[addr_p5+1] = " << (int)I_du[addr_p5+1] << endl;
    desc_addr[k++] = I_dv[addr_p5+1]; //cout << endl << "I_dv[addr_p5+1] = " << (int)I_dv[addr_p5+1] << endl;
    desc_addr[k++] = I_du[addr_m3-5]; //cout << endl << "I_du[addr_m3-5] = " << (int)I_du[addr_m3-5] << endl;
    desc_addr[k++] = I_dv[addr_m3-5]; //cout << endl << "I_dv[addr_m3-5] = " << (int)I_dv[addr_m3-5] << endl;
    desc_addr[k++] = I_du[addr_p3-5]; //cout << endl << "I_du[addr_p3-5] = " << (int)I_du[addr_p3-5] << endl;
    desc_addr[k++] = I_dv[addr_p3-5]; //cout << endl << "I_dv[addr_p3-5] = " << (int)I_dv[addr_p3-5] << endl;
    desc_addr[k++] = I_du[addr_m3+5]; //cout << endl << "I_du[addr_m3+5] = " << (int)I_du[addr_m3+5] << endl;
    desc_addr[k++] = I_dv[addr_m3+5]; //cout << endl << "I_dv[addr_m3+5] = " << (int)I_dv[addr_m3+5] << endl;
    desc_addr[k++] = I_du[addr_p3+5]; //cout << endl << "I_du[addr_p3+5] = " << (int)I_du[addr_p3+5] << endl;
    desc_addr[k++] = I_dv[addr_p3+5]; //cout << endl << "I_dv[addr_p3+5] = " << (int)I_dv[addr_p3+5] << endl;
...
void Matcher::computeDescriptors (uint8_t* I_du,uint8_t* I_dv,const int32_t bpl,std::vector<Matcher::maximum> &maxima) {
    ...
    for (vector<Matcher::maximum>::iterator it=maxima.begin(); it!=maxima.end(); it++) {
        u = (*it).u;
        v = (*it).v;
        //cout << "descriptors = " << (int32_t)Matcher::maximum(*it).d2 << endl;
        desc_addr = (uint8_t*)(&((*it).d1)); /*actually it is the begin  of d1 (class maxim)*/
...
void Matcher::computeFeatures (uint8_t *I,const int32_t* dims,int32_t* &max1,int32_t &num1,int32_t* &max2,int32_t &num2,uint8_t* &I_du,uint8_t* &I_dv,uint8_t* &I_du_full,uint8_t* &I_dv_full,int saveFeatureFlag) {
    ...
    int32_t dims_matching[3]; /*width height width*/
    memcpy(dims_matching,dims,3*sizeof(int32_t));

    //    cout << endl << "dims[2] * dims[1] = " << dims[2] << " * " << dims[1] << endl;
    //    cout << endl << "dims_matching " << dims_matching[2] << " " << dims_matching[1] << endl;
    ...
    if (!param.half_resolution) {
        ...
        I_du      = (uint8_t*)_mm_malloc(dims_matching[2]*dims_matching[1]*sizeof(uint8_t*),16);  //half resolution
        I_dv      = (uint8_t*)_mm_malloc(dims_matching[2]*dims_matching[1]*sizeof(uint8_t*),16);  //half resolution
        I_f1      = (int16_t*)_mm_malloc(dims_matching[2]*dims_matching[1]*sizeof(int16_t),16);   //half resolution
        I_f2      = (int16_t*)_mm_malloc(dims_matching[2]*dims_matching[1]*sizeof(int16_t),16);   //half resolution
        I_du_full = (uint8_t*)_mm_malloc(dims[2]*dims[1]*sizeof(uint8_t*),16);
        I_dv_full = (uint8_t*)_mm_malloc(dims[2]*dims[1]*sizeof(uint8_t*),16);

//        cv::Mat test(dims_matching[1],dims_matching[2],CV_8UC1,I_f1);
//        cv::imshow("test",test);
//        cv::waitKey(0);

        /*now we got the sobel and filter image*/
...
//    cout << endl << "maxima2.size = " << maxima2.size() << endl;
//    cv::waitKey(0);
...
//    cout << "uuu = " << maxima1[1].u << endl;
//    cout << "vvv = " << maxima1[1].v << endl;
//    cout << "d1 = " << (int32_t)maxima1[1].d1 << endl;
//    cout << "d3 = " << (int32_t)maxima1[1].d3 << endl;
//    /*actually the feature was put in the pointer max1*/
//    /*now we try to access the value */
//    int32_t stepsize = sizeof(maxima1[0])/sizeof(int32_t);
//    cout << "u = " << (int32_t)*(max1+stepsize) << endl;
//    cout << "sizeof(maxima1[0])+1) = " << sizeof(maxima1[0]) << endl;
//    cout << "v = " << (int32_t)*(max1+stepsize + 1) << endl;
//    cv::waitKey(0);
