...
std::vector<Frame> getCandidateFrames(vector<Frame>& vFrames, Frame &currentFrame, map<int, vector<int> > &edgeCandidatesIndex);
...
int main(int argc, char *argv[])
{
    ...
    ImageReader im;
    /*get the edges candidates!!*/
    map< int,vector<int> > candidates_index = getEdegsCandidateIndex();
...
//        Mat Right = cv::imread(right_img_file_name,CV_LOAD_IMAGE_GRAYSCALE);
        ...
        if(i > 0)
        {
            vector<Frame> vCandidateFrames = getCandidateFrames(vFrames,currentFrame,candidates_index);
...
std::vector<Frame> getCandidateFrames(vector<Frame>& vFrames, Frame &currentFrame, map< int,vector<int> >& edgeCandidatesIndex)
{
    ...
    int curr_id = currentFrame.mnId;
    vector<int> index;

    if(edgeCandidatesIndex.count(curr_id))
    {
        index = edgeCandidatesIndex[curr_id];
    }
    else
    {
        cout << "Something Wrong! May be you need check the config/Edge_Candidates_index.txt file " << endl;
    }

    for(int i = 0; i < index.size(); i++)
    {
        Frame cantidate(vFrames[index[i]]);
        candidates.push_back(cantidate);
