...
//    cout << "id " << currentFrameIndex << " " << lastFrameIndex << endl;
    ...
    ofstream outFile;
    outFile.open("../result/motionestimate.txt");
    ...
    cv::Mat Tcl_motionestimate = cv::Mat::eye(4,4,CV_32FC1);

    for(int32_t i = 0; i < 4541; i++)
    {

        cout << BOLDGREEN << "Process:" << i << endl;
        ...
        if(i > 1)
        {
            ...
            cv::Mat M_Rcl = Converter::toCvMat(motionEstimate2.getQuaternion());
            cv::Mat M_tcl = Converter::toCvMat(motionEstimate2.getTranslation());
            ...
            cv::imshow("reprojection-ceres", output[0]);
...
//            cout << RESET"inliers = " << result.inliers << endl;
            ...
            cout << BOLDCYAN"PNP Motion Estimate:\n" << T_pnp << endl;
